...
# load data
ps_data <- read.csv(file.path("C:/Users/sren/Dropbox (Partners HealthCare)/BOC shared/Chemo during pregnancy (Sella)/data", "ps_data.csv"))
...
taxol_coefs %>%
  ...
  kable(
    digits = 2,
    col.names = c("Covariate", "Estimated odds ratio", "Lower 95% CI", "Upper 95% CI", "P-value"),
...
mice_fit <- {
  .sub_data <- ps_data[, .SD,
    .SDcols = c(tbl1_vars, "obstetrical", "gestational")
  ]
  # mice object
  .fit <- mice(
    data = .sub_data,
...
ATE_composite <- sapply(
  c("obstetrical", "gestational", "obstetrical_sens", "gestational_sens"), function(outc) {
...
ATE_overall %>%
  ...
  kable(
    digits = 2, col.names = c("Outcome", "Estimated odds ratio", "Lower 95% CI", "Upper 95% CI"),
